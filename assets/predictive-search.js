...
const predictiveWrapper = document.getElementById('PredictiveWrapper');
const closeButtons = document.querySelectorAll('.btn--close-search');
const siteHeader = document.querySelector('.site-header__drawers-container');
const inputHeader = document.querySelector('#SiteHeader input[type="search"]');

// On typing
this.input.addEventListener('keydown', () => {
  this.classList.add('is-active');
  searchResultsWrapper.classList.add('hide');
  predictiveWrapper.classList.add('hide');

  // Remove active class from all close buttons
  closeButtons.forEach((btn) => btn.classList.remove('is-active'));
...
inputHeader.addEventListener('click', () => {
  // Add active class to all close buttons
  closeButtons.forEach((btn) => btn.classList.add('is-active'));

  siteHeader.classList.add('is-active');
  searchResultsWrapper.classList.remove('hide');
  predictiveWrapper.classList.remove('hide');
});

// On any close button click
closeButtons.forEach((btn) => {
  btn.addEventListener('click', function () {
    searchResultsWrapper.classList.add('hide');
    predictiveWrapper.classList.add('hide');

    // Remove is-active from all close buttons
    closeButtons.forEach((b) => b.classList.remove('is-active'));
